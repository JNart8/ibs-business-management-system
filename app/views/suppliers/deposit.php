...
<div class="max-w-lg mx-auto">
    ...
    <!-- Payment Form -->
    <form method="POST" action="<?= BASE_URL ?>/suppliers/deposit/<?= $supplier['id'] ?>"
        class="bg-white rounded-lg shadow p-6"
        x-data="supplierPayForm()">
        <?= csrfField() ?>

        <h1 class="text-xl font-bold text-gray-800 mb-5">Record Supplier Payment</h1>
        ...
        <div class="mb-5">
            ...
            <p class="text-xs text-gray-400 mt-1">
                Outstanding balance: <strong class="text-red-500"><?= formatMoney(abs($balance)) ?></strong>
            </p>
            <?php endif; ?>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="date"
                ...
                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg
                          focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <p class="text-xs text-gray-500 mt-1">Back-date this payment if it was made on a different day.</p>
        </div>

        <!-- Balance Preview -->
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6" x-show="amount > 0" x-cloak>
            <div class="flex justify-between items-center mb-2">
                <span class="text-sm text-gray-600">New Supplier Balance After Payment:</span>
                <span class="font-bold text-lg" :class="newBalanceNum >= 0 ? 'text-green-600' : 'text-orange-500'" x-text="newBalanceDisplay"></span>
            </div>
...
<script>
    const accountBalances = <?= json_encode(array_column($accounts ?? [], 'balance', 'id')) ?>;
    ...
    function supplierPayForm() {
        return {
            amount: 0,
            selectedAccountId: '<?= isset($accounts[0]) ? $accounts[0]['id'] : '' ?>',
            newBalanceNum: currentSupplierBalance,
            newBalanceDisplay: fmt(currentSupplierBalance),
            remainingDisplay: '',
            calcNewBalance() {
                const amt = parseFloat(this.amount) || 0;
                const nb = currentSupplierBalance + amt;

...
<div class="max-w-lg mx-auto">
    ...
    <!-- Deposit Form -->
    <form method="POST" action="<?= BASE_URL ?>/customers/deposit/<?= $customer['id'] ?>"
        class="bg-white rounded-lg shadow p-6"
        x-data="depositForm()">
        <?= csrfField() ?>

        <h1 class="text-xl font-bold text-gray-800 mb-5">Record Deposit</h1>
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6" x-show="amount > 0" x-cloak>
            <div class="flex justify-between items-center">
                <span class="text-sm text-gray-600">New Balance After Deposit:</span>
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700 mb-1">
                📅 Deposit Date <span class="text-xs text-gray-400 font-normal">(defaults to today)</span>
            </label>
            <input type="date"
                name="deposit_date"
                id="deposit_date"
                ...
                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg
                          focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <p class="text-xs text-gray-500 mt-1">Back-date this deposit if it was received on a different day.</p>
        </div>
        ...
        <div class="flex justify-between">
            ...
            <button type="submit" <?= empty($accounts) ? 'disabled' : '' ?>
                class="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow transition

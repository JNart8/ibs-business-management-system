...
<div class="h-screen-safe" x-data="purchaseForm()" x-init="init()">
    ...
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold text-gray-800">📦 Create Purchase</h1>
        ...
        <div class="lg:col-span-2 lg:sticky lg:top-4" style="align-self: flex-start;">
            <div class="bg-white rounded-lg shadow flex flex-col" style="min-height: 580px; max-height: 88vh;">
                ...
                <div class="border-t px-4 py-3 space-y-3">
                    ...
                    <!-- Purchase Date Override -->
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Purchase Date (Optional)</label>
                        <input type="date"
                            x-model="customPurchaseDate"
                            max="<?= date('Y-m-d') ?>"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                                      focus:ring-2 focus:ring-blue-500">
                    </div>
...
<script>
    function purchaseForm() {
        return {
            ...
            selectedAccountId: 0,
            preselectedProduct: <?= json_encode($preselectedProduct) ?>,
            ...
            init() {
                ...
                // Auto-select the default account for the initial method
                this.autoSelectAccount(this.paymentMethod);

                // Handle preselected product from stock-in redirect
                if (this.preselectedProduct) {
                    if (this.preselectedProduct.supplier_id) {
                        this.supplierId = this.preselectedProduct.supplier_id;
                        this.selectSupplier();
                    }
                    this.addToCart(this.preselectedProduct);
                } else {
                    // Handle preselected supplier from URL parameter
                    const urlParams = new URLSearchParams(window.location.search);
                    const supplierParam = urlParams.get('supplier_id') || urlParams.get('supplier');
                    if (supplierParam) {
                        this.supplierId = supplierParam;
                        this.$nextTick(() => {
                            this.selectSupplier();
                        });
                    }
                ...
                fetch('<?= BASE_URL ?>/purchases/complete', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            window.location.href = '<?= BASE_URL ?>/purchases/view/' + data.purchase_id;
                        } else {
